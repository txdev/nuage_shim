...
from nuage_shim.backends.nuage_net_l3vpn import NuageNetL3VPN
from nuage_shim.api_models.net_l3vpn import ApiNetL3VPN
...
def setup_app():
    ...
    #
    backend = NuageNetL3VPN(CONF.shim.vsd_url,
                            CONF.shim.vsd_username,
                            CONF.shim.vsd_password,
                            CONF.shim.vsd_csp_enterprise,
                            CONF.shim.vsd_app_enterprise)
...
def main():
    SHIM_OPTS = [
        cfg.StrOpt('vsd_url',
                   ...
                   help='VSD API URL'),
        cfg.StrOpt('vsd_username',
                   default='csproot',
                   help='VSD Username'),
        cfg.StrOpt('vsd_password',
                   default='csproot',
                   help='VSD Password'),
        cfg.StrOpt('vsd_csp_enterprise',
                   default='csp',
                   help='VSD CSP Enterprise'),
        cfg.StrOpt('vsd_app_enterprise',
                   default='Gluon',
                   help='VSD Appp Enterprise'),
        ...
        cfg.StrOpt('host_list',
                   ...
                   help='Comma separated list of hostnames managed by this server'),
    ]
    ...
    ShimData.client = etcd.Client(host=ShimData.etcd_host, port=ShimData.etcd_port, read_timeout=3600)
    setup_app()
    ...
    while True:
        ...
        try:
            ...
            if wait_index:
                message = ShimData.client.read(ShimData.proton_etcd_dir, recursive=True, wait=True, waitIndex=wait_index)

            else:
                message = ShimData.client.read(ShimData.proton_etcd_dir, recursive=True, wait=True)
            ...
            LOG.error("Cannot connect to etcd, make sure that etcd is running. Trying in 5 seconds")
            time.sleep(5)
